...
import com.android.painton.ui.adapters.ColorViewAdapter
import com.android.painton.utils.ColorUtils.dip2px
import com.android.painton.utils.ColorUtils.getDimensionDp
...
class ColorPicker(context: Activity) {
    ...
    private var dialogViewLayout: View
    ...
    fun setColors(resId: Int): ColorPicker {
        ...
        for (i in 0 until (ta?.length() ?: 0)) {
            colors?.add(ColorPal(ta?.getColor(i, 0) ?: 0, false))
        }
        return this
    }
    ...
    fun show() {
        ...
        if (colors == null || colors?.isEmpty() == true) setColors()
        val titleView: AppCompatTextView = dialogViewLayout.findViewById(R.id.title)
        if (title != null) {
            titleView.text = title
            titleView.setPadding(
                dip2px(paddingTitleLeft.toFloat(), context), dip2px(paddingTitleTop.toFloat(), context),
                dip2px(paddingTitleRight.toFloat(), context), dip2px(paddingTitleBottom.toFloat(), context)
            )
        }
        mDialog = WeakReference<CustomDialog>(CustomDialog(context, dialogViewLayout))
        val gridLayoutManager = GridLayoutManager(context, columns)
        recyclerView.layoutManager = gridLayoutManager
        colorViewAdapter =
            if (fastChooser) ColorViewAdapter(colors ?: arrayListOf(), onFastChooseColorListener, mDialog) else ColorViewAdapter(colors ?: arrayListOf())
        if (fullHeight) {
        ...
        if (marginColorButtonBottom != 0 || marginColorButtonLeft != 0 || marginColorButtonRight != 0 || marginColorButtonTop != 0) {
            colorViewAdapter?.setColorButtonMargin(
                dip2px(marginColorButtonLeft.toFloat(), context), dip2px(marginColorButtonTop.toFloat(), context),
                dip2px(marginColorButtonRight.toFloat(), context), dip2px(marginColorButtonBottom.toFloat(), context)
            )
        }
        ...
        positiveButton.setOnClickListener {
            if (onChooseColorListener != null && !fastChooser) onChooseColorListener?.onChooseColor(
                colorViewAdapter?.colorPosition ?: 0,
                colorViewAdapter?.colorSelected ?: 0
            )
            if (dismiss) {
                dismissDialog()
        ...
        val dialog: CustomDialog? = mDialog?.get()
     ...
     * @return this
     */
    fun addListenerButton(text: String?, listener: OnButtonListener): ColorPicker {
        if (mContext == null) return this
        val context = mContext.get() ?: return this
        ...
        button.minWidth = getDimensionDp(R.dimen.action_button_min_width, context)
        button.minimumWidth = getDimensionDp(R.dimen.action_button_min_width, context)
        button.setPadding(
            getDimensionDp(R.dimen.action_button_padding_horizontal, context) + dip2px(5f, context),
            0,
            getDimensionDp(R.dimen.action_button_padding_horizontal, context) + dip2px(5f, context),
            0
        )
        button.setBackgroundResource(R.drawable.button)
        button.textSize = getDimensionDp(R.dimen.action_button_text_size, context).toFloat()
